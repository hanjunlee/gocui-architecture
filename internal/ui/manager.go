...
import (
	...
	viewutil "github.com/hanjunlee/gocui-architecture/pkg/util/view"
...
type ManagerIface interface {
	...
	ReturnSwitcher(g *gocui.Gui) (service.UseCase, error)
}
...
func (m *Manager) Layout(g *gocui.Gui) error {
	...
	v.Clear()
	dect := m.Dected
	for _, d := range m.Svc.Search(dect) {
...
func (m *Manager) ReturnSwitcher(g *gocui.Gui) (service.UseCase, error) {
	v, _ := g.View(Switcher)
	s, _ := v.Line(0)
	s = strings.TrimSpace(s)
